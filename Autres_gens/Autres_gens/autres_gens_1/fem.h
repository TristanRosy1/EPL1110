...
typedef enum { DIRICHLET_X, DIRICHLET_Y, DIRICHLET_XY, DIRICHLET_N, DIRICHLET_T, DIRICHLET_NT, NEUMANN_X, NEUMANN_Y, NEUMANN_N, NEUMANN_T,NEUMANN_WIND, UNDEFINED=-1} femBoundaryType;
typedef enum { PLANAR_STRESS, PLANAR_STRAIN, AXISYM } femElasticCase;
// ce qu'on a rajouté 
// renumération des noeuds 
...
typedef struct {
  ...
  double *X;
  double *Y;
  //ajout de la numération des noeuds
...
void femSystemConstrain(femBandSystem *mySystem, int myNode, double value);
void femBandSystemAssemble(femBandSystem *mySystem, double *Aloc, double *Bloc, int *map, int nLoc);
...
// fonctions ajoutées pour la renumération des noeuds
void femRenumNodes(femMesh *theMesh, femRenumType renumType);

...
void geoMeshRead(const char *filename) {
  ...
  // ajout de la numération des noeuds ici dans un ordre arbitraire croissant 
...
void femSystemConstrain(femBandSystem *mySystem, int myNode, double myValue) {
  //transforamtion du système algébrique et système réduit 
  double **A, *B;
  int i, size;
  // calcul de la matrice de raideur Aij et forces nodales Bi
  A = mySystem->A;
  B = mySystem->B;
  size = mySystem->size;
...
femProblem *femElasticityCreate(femGeo *theGeometry, double E, double nu, double rho, double gx, double gy, femElasticCase iCase) {
  ...
  theProblem->system = femBandSystemCreate(size,size);
  return theProblem;
}

void femElasticityFree(femProblem *theProblem) {
  femBandSystemFree(theProblem->system);
...
void femElasticityPrint(femProblem *theProblem) {
  ...
  for (int i = 0; i < theProblem->nBoundaryConditions; i++) {
    ...
    if (theCondition->type == NEUMANN_WIND)
      printf(" imposing %9.2e as the force of the wind \n", value1);
  }
  printf(" ======================================================================================= \n\n");
}
...
void femElasticityWrite(femProblem *theProblem, const char *filename) {
  ...
  for (int i = 0; i < theProblem->nBoundaryConditions; i++) {
    ...
    case NEUMANN_WIND:
      fprintf(file, " Neumann-Wind       = %14.7e, %14.7e ", value1, NAN);
      break;
    default:
...
femProblem *femElasticityRead(femGeo *theGeometry, const char *filename) {
  ...
  while (!feof(file)) {
    ...
    if (strncasecmp(theLine, "Boundary condition  ", 19) == 0) {
      ...
      if (strncasecmp(theArgument, "Neumann-Wind", 19) == 0)
        typeCondition = NEUMANN_WIND;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////
//////Fonctions utilisées pour la renumeration des noeuds avc la méthode de reverse Cuthill-McKee///////
////////////////////////////////////////////////////////////////////////////////////////////////////////
/**
 * @brief Structure représentant uen queue de noeuds.
 * 
 * Cette structure contient les noeuds de notre mesh.
 */
Queue* createQueue() {
...
/**
 * @brief Ajout d'un noeud en début de queue.
 * 
 * Cette fonction prend une valeur et un queue en entrée et modifie la queue en rajoutant au debut cette nouvelle valeur..
 * 
 * @param queue la queue dans laquelle on travaille.
 * @param value la valeur a rajoutée.
...
/**
 * @brief Enleve et renvoie la valeur du dernier élément de la queue.
 * 
 * Cette fonction prend la queue en entrée et retourne la valeur du dernier élément qui est retiré de cette queue.
 * 
 * @param queue la queue dont on veut retirer le dernier élément.
 * @return La valeur du dernier élément qui est retiré.
 */
...
/**
 * @brief Structure représentant un graphe non orienté pour modéliser la matrice creuse.
 * 
 * Cette structure contient une taille de graphe, degres des noeuds et les voisins de chaque noeud.
 * 
 */
Graph* createGraph(int size){
    Graph* graph = (Graph*)malloc(sizeof(Graph));
    graph->size = size;
    graph->degrees = (int*)malloc(size*sizeof(int));
    graph->neighbours = (int**)malloc(size*sizeof(int*));
    for(int i = 0; i < size; i++){
        graph->degrees[i] = 0;
        graph->neighbours[i] = (int*)malloc(size*sizeof(int));
    }
    return graph;
}

/**
 * @brief Ajout d'une arête entre deux noeuds.
 * 
 * Cette fonction prend un graphe et deux noeuds en entrée et ajoute une arête entre ces deux noeuds.
 * 
 * @param graph le graphe dans lequel on travaille.
 * @param u le premier noeud.
 * @param v le deuxième noeud.
 * @return void.
 */
void addEdge(Graph* graph, int u, int v){
    graph->neighbours[u][graph->degrees[u]] = v;
    graph->degrees[u]++;
}
 ...
 * 
 * Cette fonction prend un graphe en entrée et libère la mémoire allouée pour ce graphe.
 * 
...
/**
 * @brief Retourne les voisins d'un noeud dans l'ordre croissant de degré.
 * 
 * Cette fonction prend un noeud, un graphe et une taille en entrée et retourne les voisins de ce noeud dans l'ordre croissant de degré.
 * 
 * @param node le noeud dont on veut les voisins.
 * @param graph le graphe dans lequel on travaille.
 * @param size la taille du graphe.
 * @return Les voisins du noeud dans l'ordre croissant de degré.
 */
int* getOrderedNeighbours(int node, Graph* graph, int size){
    int* orderedNeighbours = (int*)malloc(graph->degrees[node]*sizeof(int));
    int* neighbours = graph->neighbours[node];
    int* degrees = graph->degrees;
    int* visited = (int*)calloc(graph->size, sizeof(int)); // Initialize visited array
    for(int i = 0; i < degrees[node]; i++){
        int minDegree = size; // Initialise à une valeur maximale
        int minNode = -1; // Initialise à une valeur non valide
        for(int j = 0; j < degrees[node]; j++){
            if(!visited[neighbours[j]] && degrees[neighbours[j]] < minDegree){
                minDegree = degrees[neighbours[j]];
                minNode = neighbours[j];
            }
        }
        orderedNeighbours[i] = minNode;
        visited[minNode] = 1; // Marque le nœud comme visité
    }
    free(visited); // Libére le tableau visité
    return orderedNeighbours;
}

////////////////////////////////////////////////////////////////////////////
////Implémentation de l'algorithme de Reverse Cuthill-McKee////////////////
///////////////////////////////////////////////////////////////////////////
/**
 * @brief Reverse Cuthill-McKee algorithm pour reordonné des matrices creuses.
 * La méthode reverse Cuthill-McKee est un algorritme basé sur les graphes utilisé pour réordonner les noeuds 
 * d'une matrice creuse de manière à réduire la bande de la matrice résultante. Il est couramment utilisé dans 
 * les calculs numériques pour améliorer les performances du cache et réduire la quantité de remplissage lors de
 * la factorisation de la matrice.
 * 
 * Cette implémentation prend une matrice d'entrée représentée sous forme de liste d'adjacence et renvoie un
 * vecteur de permutation qui réordonne les noeuds de la matrice. Le vecteur de permutation peut ensuite être
 * utilisé pour réordonner les lignes et les colonnes de la matrice afin de réduire sa bande.
 *
 * @param matrix La matrice d'entrée.
 * @param size La taille de la matrice.
 * @return Le vecteur de permutation qui reordonne les noeuds de notre matrice.
 */
int* reverseCuthillMackee(double** matrix, int size){
    // Création du graphe
    ...
    while(True == 1){
        ...
        // On ajoute le noeud de degré minimum
        int minDegree = size;
        int minNode = 0;
        int check = 0;
        for(int i = 0; i < size; i++){
            if(degrees[i] < minDegree && degrees[i] != -1){
            check = 1;
            minDegree = degrees[i];
            minNode = i;
            }
        ...
        enqueue(queue, minNode);
        // On parcourt les noeuds de la file
        while(queue->head != NULL){
            if (degrees[queue->head->value] == -1){
                int node = dequeue(queue);
            }else{
                int node = dequeue(queue);
                permutation[index] = node;

                // On parcourt les voisins du noeud et on les ajoute à la queue si ils n'ont pas encore été traités et dans l'ordre croissant de degré
                int* orderedNeighbours = getOrderedNeighbours(node, graph, size);
                for(int i = 0; i < graph->degrees[node]; i++){
                    int neighbour = orderedNeighbours[i];
                    if(degrees[neighbour] != -1){
                        enqueue(queue, neighbour);
                    }
                }
                index++;
                free(orderedNeighbours);
                degrees[node] = -1;
            }
        }
        free(queue);
...
/**
 * @brief Libère la mémoire allouée pour un problème.
 * 
 * Cette fonction prend un problème et sa taille en entrée et libère la mémoire allouée pour ce problème.
 * 
 * @param problem le problème à libérer.
 * @param size la taille du problème.
 * @return void.
 */
void freeProblem(problem *problem, int size){
    for (int i = 0; i < size; i++){
        free(problem->A[i]);
    }
    free(problem->A);
...
// On renumérote le mesh en fonction de la permutation
// Le nouveau numéro du noeud N est i dans permutation[i] = N
/**
 * @brief Renumérote les noeuds de notre mesh.
 * 
 * Cette fonction prend une matrice, un vecteur de permutation et une taille en entrée et renumérote les noeuds de notre mesh.
 * 
 * @param A la matrice à renuméroter.
 * @param B le vecteur de permutation.
 * @param perm la taille de la matrice.
 * @param size la taille du vecteur de permutation.
 * @return Un problème renuméroté.
 */
problem *renumberMesh(double** A, double* B, int* perm, int size){
    double **Aperm = malloc(size*sizeof(double*));
    for (int i = 0; i < size; ++i){
        Aperm[i] = malloc(size*sizeof(double));
    }
    for (int i = 0; i < size; ++i){
        for (int j = 0; j < size; ++j){
    ...
    double *Bperm = malloc(size*sizeof(double));
    for (int i = 0; i < size; ++i){
        Bperm[i] = B[perm[i]];
    }
    problem *newProblem = malloc(sizeof(problem));
    newProblem->A = Aperm;
    newProblem->B = Bperm;
    return newProblem;
}
...
/**
 * @brief Inverse la permutation des noeuds.
 * 
 * Cette fonction prend un vecteur de permutation, un vecteur de valeurs et une taille en entrée et inverse la permutation des noeuds.
 * 
 * @param b le vecteur de valeurs.
 * @param perm le vecteur de permutation.
 * @param size la taille des vecteurs.
 * @return Un vecteur de valeurs inversées.
 */
double* inversePermutation(double*b, int* perm, int size){
    int *invPerm = malloc(size*sizeof(int));
    for (int i = 0; i < size; ++i){
        invPerm[perm[i]] = i;
    }
...
/**
 * @brief Calcule la largeur de bande d'une matrice.
 * 
 * Cette fonction prend une matrice et sa taille en entrée et retourne la largeur de bande de cette matrice.
 * 
 * @param A la matrice dont on veut calculer la largeur de bande.
 * @param size la taille de la matrice.
 * @return La largeur de bande de la matrice.
 */
int matrixComputeBand(double **A, int size){
    int band = 0;
    for (int i = 0; i < size; ++i){
        for (int j = 0; j < size; ++j){
            if (A[i][j] != 0 && abs(i-j) > band){
                band = abs(i-j);
            }
        }
    }
